...
/** Compute the inner node hash with domain separation and delimiter. */
...
export function buildMerkleRoot(leafHashes: string[]): string | null {
  ...
  if (leafHashes.length === 1) return nodeHash(leafHashes[0], leafHashes[0])
 ...
 * Builds the tree once and extracts both root and sibling path in a single pass.
 */
export function buildInclusionProof(leafHashes: string[], index: number): MerkleProof | null {
  ...
  if (leafHashes.length === 1) {
    const root = nodeHash(leafHashes[0], leafHashes[0])
    return { leafIndex: 0, leafHash: leafHashes[0], siblings: [leafHashes[0]], root }
  }
