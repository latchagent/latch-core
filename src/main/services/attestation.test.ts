...
function makeInput(overrides: Partial<ReceiptInput> = {}): ReceiptInput {
  return {
    ...
    servicesUsed: ['github'],
    activity: { requests: 10, blocked: 1, redactions: 0, tokenizations: 0, toolCalls: 5, toolDenials: 0, approvalEscalations: 0 },
    sandboxType: 'docker',
    networkForced: true,
    exitReason: 'normal',
...
describe('AttestationEngine', () => {
  ...
  it('saves receipt to store', () => {
    engine.generateReceipt({
      ...
      servicesGranted: [],
      servicesUsed: [],
      activity: { requests: 0, blocked: 0, redactions: 0, tokenizations: 0, toolCalls: 0, toolDenials: 0, approvalEscalations: 0 },
      sandboxType: 'docker',
      networkForced: true,
      exitReason: 'normal',
      startTime: Date.now(),
