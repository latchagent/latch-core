...
//        shell.env (env injection), session lifecycle (status reporting).
...
export const LatchPolicy = async (ctx) => {
  return {
    ...
    'tool.execute.after': async (input, output) => {
      ...
      const detail = args.filePath || args.file_path || args.command || '';
    ...
    'file.edited': async ({ event }) => {
      await postFeed(\`File edited: \${event?.path ?? 'unknown'}\`);
    },
  };
};
`
