...
import type { PolicyDocument, PolicyPermissions, CodexPolicyConfig, OpenCodePolicyConfig, HarnessesConfig, ToolRule, McpServerRule, CommandRule } from '../../types'
...
export function enforceForClaude(
  ...
  if (authzOptions) {
    ...
    // PostToolUse hook: reports agent activity to the Latch feed.
    // Reads tool_name + tool_input from stdin, extracts a brief label,
    // and POSTs it to $LATCH_FEED_URL (env var set on PTY creation).
    // Fails silently — feed reporting should never block the agent.
    const feedScriptPath = path.join(claudeDir, 'latch-feed.sh')
    const feedScriptContent = [
      '#!/bin/bash',
      '# Generated by Latch Desktop — do not edit manually.',
      '# PostToolUse hook: reports tool activity to the Latch feed.',
      '[ -z "$LATCH_FEED_URL" ] && exit 0',
      '',
      '# Read tool info from stdin',
      'INPUT=$(cat)',
      'TOOL=$(printf \'%s\' "$INPUT" | grep -o \'"tool_name":"[^"]*"\' | head -1 | sed \'s/"tool_name":"//;s/"$//\')',
      '[ -z "$TOOL" ] && exit 0',
      '',
      '# Extract a brief context hint from tool_input',
      'case "$TOOL" in',
      '  Bash)',
      '    HINT=$(printf \'%s\' "$INPUT" | grep -o \'"command":"[^"]*"\' | head -1 | sed \'s/"command":"//;s/"$//\' | cut -c1-60) ;;',
      '  Write|Edit|Read)',
      '    HINT=$(printf \'%s\' "$INPUT" | grep -o \'"file_path":"[^"]*"\' | head -1 | sed \'s/"file_path":"//;s/"$//\' | xargs basename 2>/dev/null) ;;',
      '  *)',
      '    HINT="" ;;',
      'esac',
      '',
      ...
      'fi',
      '',
      '# POST to feed (fire-and-forget, fail silently)',
      'curl -sf "$LATCH_FEED_URL" \\',
      '  -H "Content-Type: application/json" \\',
      '  --connect-timeout 2 --max-time 3 \\',
      '  -d "{\\"status\\":\\"$MSG\\"}" > /dev/null 2>&1 &',
      'exit 0',
      '',
    ].join('\n')

    fs.writeFileSync(feedScriptPath, feedScriptContent, 'utf-8')
    try { fs.chmodSync(feedScriptPath, 0o755) } catch { /* Windows — non-fatal */ }
    ...
    existing.hooks = {
      ...
      PostToolUse: [
        {
          matcher: '',
          hooks: [{ type: 'command', command: `bash '${feedScriptPath}'` }],
        },
      ],
    }
  }
...
function buildOpenCodePermissions(p: PolicyPermissions): Record<string, unknown> {
  ...
  if (!p.allowFileWrite) {
    perms.edit = 'deny'
  } else if (p.blockedGlobs?.length) {
    const editRules: Record<string, string> = { '*': 'allow' }
    for (const glob of p.blockedGlobs) editRules[glob] = 'deny'
  ...
  if (p.blockedGlobs?.length) {
    ...
    for (const glob of p.blockedGlobs) readRules[glob] = 'deny'
...
export function generateOpenCodePlugin(
  ...
  const feedUrl = `http://127.0.0.1:${authzOptions.port}/feed/${authzOptions.sessionId}`

  const pluginSrc = `// Generated by Latch Desktop — do not edit manually.
// Runtime policy enforcement plugin for OpenCode.
// Hooks: tool.execute.before (authz gate), tool.execute.after (feed),
//        shell.env (env injection), event (session + message lifecycle).
...
const EVENT_URL = \`http://127.0.0.1:\${AUTHZ_PORT}/opencode-event/\${SESSION_ID}\`;

let turnIndex = 0;
let lastRole = '';

/** POST to Latch feed endpoint (fire-and-forget). */
async function postFeed(status) {
  try {
...
/** POST a conversation event to Latch (fire-and-forget). */
...
export const LatchPolicy = async (ctx) => {
  return {
    'tool.execute.before': async (input, output) => {
      try {
        ...
        const res = await fetch(SUPERVISE_URL, {
          ...
          body: JSON.stringify({ tool: toolName, input: output.args ?? {} }),
        ...
        // Re-throw policy blocks, fail open on network errors
        if (err?.message?.startsWith('Blocked by Latch')) throw err;
      }
    },

    'tool.execute.after': async (input, output) => {
      const toolName = input.tool ?? 'unknown';
      const detail = output.title || '';
      const label = detail ? \`\${toolName}: \${String(detail).slice(0, 60)}\` : toolName;
      await postFeed(label);
    },

    'shell.env': async (input, output) => {
      output.env.LATCH_SESSION_ID = SESSION_ID;
      output.env.LATCH_HARNESS_ID = 'opencode';
      output.env.LATCH_FEED_URL = FEED_URL;
    },
    ...
    'event': async ({ event }) => {
      ...
      switch (event.type) {
        case 'session.idle':
          turnIndex++;
          await postFeed('Waiting for input');
          break;

        case 'session.error':
          await postFeed('Session error');
          break;

        case 'file.edited':
          await postFeed(\`File edited: \${props.file ?? 'unknown'}\`);
          break;
        ...
        case 'message.updated': {
          ...
          if (info.role === 'assistant') {
            ...
            turnIndex++;
            await postEvent({ kind: 'prompt', turnIndex });
          }
          break;
        }
        ...
        case 'message.part.updated': {
          ...
          if (part.type === 'tool') {
            ...
            if (!state || (state.status !== 'completed' && state.status !== 'error')) break;
            const toolName = part.tool ?? 'unknown';
            const input = state.input ?? {};
            const inputStr = input.filePath || input.file_path || input.command || input.pattern || '';
            ...
            await postEvent({
              ...
              turnIndex,
              toolName,
              toolInput: String(inputStr).slice(0, 120),
              toolResult: String(result).slice(0, 2000),
              isError,
            });
          } else if (part.type === 'text' && part.text) {
            await postEvent({
              kind: lastRole === 'user' ? 'prompt' : 'response',
              turnIndex,
              textContent: String(part.text).slice(0, 2000),
            });
...
const LATCH_CONN_PATH = path.join(
  os.homedir(),
  '.config', 'opencode', 'latch-connection.json',
)

/** Write a small JSON file that the global plugin reads at startup. */
function writeLatchConnection(port: number, secret: string): void {
  const dir = path.dirname(LATCH_CONN_PATH)
  fs.mkdirSync(dir, { recursive: true })
  fs.writeFileSync(
    LATCH_CONN_PATH,
    JSON.stringify({ port, secret }, null, 2) + '\n',
    'utf-8',
  )
}

/** Install the Latch plugin into ~/.config/opencode/plugins/ (global).
 *  This runs once when the authz server starts so every OpenCode session
 *  automatically gets conversation forwarding and feed reporting.
 *
 *  The plugin reads port/secret from ~/.config/opencode/latch-connection.json
 *  and uses the OpenCode session ID from hook inputs as the session key.
 */
export function installGlobalOpenCodePlugin(port: number, secret: string): void {
  // Write connection file first
  writeLatchConnection(port, secret)
  ...
  const pluginSrc = `// Generated by Latch Desktop — do not edit manually.
// Global OpenCode plugin: conversation event forwarding, feed, and policy hooks.
// Reads Latch authz connection from ~/.config/opencode/latch-connection.json.

import fs from 'node:fs';
import path from 'node:path';
import os from 'node:os';

const CONN_PATH = path.join(os.homedir(), '.config', 'opencode', 'latch-connection.json');

let AUTHZ_PORT = 0;
let AUTHZ_SECRET = '';

try {
  const raw = fs.readFileSync(CONN_PATH, 'utf-8');
  const conn = JSON.parse(raw);
  AUTHZ_PORT = conn.port ?? 0;
  AUTHZ_SECRET = conn.secret ?? '';
} catch {
  // Latch not running — plugin will no-op
}

let turnIndex = 0;
let lastRole = '';

// Canonical session ID — prefer the Latch session ID from env (set by PTY)
// so all events, feed, checkpoints, and live events use the same ID that
// the Latch UI expects. Falls back to the first OpenCode-internal ID.
let canonicalSessionId = process.env.LATCH_SESSION_ID || '';
let cwdRegistered = false;

function baseUrl(): string {
  return \`http://127.0.0.1:\${AUTHZ_PORT}\`;
}

/** Register CWD with Latch so the checkpoint engine knows the worktree. */
async function registerCwd(sessionId: string): Promise<void> {
  try {
    await fetch(\`\${baseUrl()}/opencode-cwd/\${sessionId}\`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': \`Bearer \${AUTHZ_SECRET}\`,
...
/** Resolve the session ID. Prefers LATCH_SESSION_ID from env (set once above).
 *  If no env ID was set (standalone OpenCode), locks to the first candidate. */
function resolveSessionId(candidate: string | undefined): string {
  if (!canonicalSessionId && candidate && candidate !== 'unknown') {
    canonicalSessionId = candidate;
  }
  // Register CWD once, after we have a canonical ID
  if (canonicalSessionId && !cwdRegistered) {
    cwdRegistered = true;
    registerCwd(canonicalSessionId);
  }
  return canonicalSessionId || 'unknown';
}

async function postFeed(sessionId: string, status: string): Promise<void> {
  if (!AUTHZ_PORT || !sessionId || sessionId === 'unknown') return;
  try {
    await fetch(\`\${baseUrl()}/feed/\${sessionId}\`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ status }),
      signal: AbortSignal.timeout(3000),
...
async function postEvent(sessionId: string, event: Record<string, unknown>): Promise<void> {
  if (!AUTHZ_PORT || !sessionId || sessionId === 'unknown') return;
  try {
    await fetch(\`\${baseUrl()}/opencode-event/\${sessionId}\`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': \`Bearer \${AUTHZ_SECRET}\`,
...
/** Extract a raw session ID from event properties or hook inputs. */
function extractSid(input: Record<string, any>): string | undefined {
  return input?.sessionID || input?.sessionId || undefined;
}

export const LatchPlugin = async (_ctx: any) => {
  // Re-read connection on plugin load in case Latch restarted
  try {
    const raw = fs.readFileSync(CONN_PATH, 'utf-8');
    const conn = JSON.parse(raw);
    AUTHZ_PORT = conn.port ?? 0;
    AUTHZ_SECRET = conn.secret ?? '';
  } catch { /* Latch not running */ }

  if (!AUTHZ_PORT) {
    // Latch Desktop is not running — return empty hooks
    return {};
  }
  ...
  return {
    'tool.execute.before': async (input: any, output: any) => {
      try {
        ...
        const sessionId = resolveSessionId(extractSid(input));
        const res = await fetch(\`\${baseUrl()}/supervise/\${sessionId}\`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': \`Bearer \${AUTHZ_SECRET}\`,
    ...
    'tool.execute.after': async (input: any, output: any) => {
      const toolName: string = input.tool ?? 'unknown';
      const detail: string = output.title || '';
      const label = detail ? \`\${toolName}: \${String(detail).slice(0, 60)}\` : toolName;
      await postFeed(resolveSessionId(extractSid(input)), label);
    },

    'shell.env': async (input: any, output: any) => {
      const sessionId = resolveSessionId(extractSid(input));
      output.env.LATCH_SESSION_ID = sessionId;
      output.env.LATCH_HARNESS_ID = 'opencode';
      output.env.LATCH_FEED_URL = \`\${baseUrl()}/feed/\${sessionId}\`;
    },

    'event': async ({ event }: { event: any }) => {
      if (!event || !event.type) return;
      const props: Record<string, any> = event.properties ?? {};
      // Resolve session ID from event properties — uses canonical ID once set
      const sessionId = resolveSessionId(props.sessionID || props.sessionId);

      switch (event.type) {
        case 'session.idle':
          await postFeed(sessionId, 'Waiting for input');
          break;

        case 'session.error':
          await postFeed(sessionId, 'Session error');
          break;

        case 'file.edited':
          await postFeed(sessionId, \`File edited: \${props.file ?? 'unknown'}\`);
          break;
        ...
        case 'message.updated': {
          ...
          if (!info) break;
          lastRole = info.role ?? '';
          // Always use canonical session ID
          if (info.role === 'assistant') {
            ...
            turnIndex++;
            await postEvent(sessionId, { kind: 'prompt', turnIndex });
          }
          break;
        }
        ...
        case 'message.part.updated': {
          ...
          if (part.type === 'tool') {
            ...
            const toolInput: Record<string, any> = state.input ?? {};
            const inputStr: string = toolInput.filePath || toolInput.file_path || toolInput.command || toolInput.pattern || '';
            const isError = state.status === 'error';
            const result = isError ? (state.error ?? '') : (typeof state.output === 'string' ? state.output : JSON.stringify(state.output ?? ''));
            await postEvent(sessionId, {
              kind: 'tool-call', turnIndex, toolName,
              toolInput: String(inputStr).slice(0, 120),
              toolResult: String(result).slice(0, 2000),
              isError,
            });
            ...
            await postEvent(sessionId, {
              kind: 'thinking', turnIndex,
              textContent: String(part.text).slice(0, 2000),
            });
          } else if (part.type === 'step-finish') {
            await postEvent(sessionId, {
              kind: 'step-finish', turnIndex,
              model: null,
              inputTokens: part.tokens?.input ?? null,
              outputTokens: part.tokens?.output ?? null,
              reasoningTokens: part.tokens?.reasoning ?? null,
...
`
  ...
  fs.writeFileSync(path.join(pluginDir, 'latch.ts'), pluginSrc, 'utf-8')
  console.log(`[opencode] Global plugin installed at ${pluginDir}/latch.ts`)
}
...
export function enforceForOpenCode(
  ...
  // Layer 2: Plugin injection is handled globally by installGlobalOpenCodePlugin()
  // which installs a single plugin at ~/.config/opencode/plugins/latch.ts.
  // No per-session local plugin needed — the global plugin reads connection
  // details from latch-connection.json and uses OpenCode's session IDs.
